function [bestc,bestg,bestcv,hC] = modsel(label,inst,fold)
...
for log2c = c_begin:c_step:c_end
    for log2g = g_begin:g_step:g_end
        ...
        cv = libsvmtrain(label,inst,cmd);
        if (cv > bestcv) || ((cv == bestcv) && (2^log2c < bestc) && (2^log2g == bestg))
            bestcv = cv; bestc = 2^log2c; bestg = 2^log2g;
        end
        disp([num2str(log2c),' ',num2str(log2g),' (Melhor c=',num2str(bestc),' g=',num2str(bestg),' acuracia=',num2str(bestcv),'%)'])
        Z(i,j) = cv;
        j = j+1;
    end
    j = 1;
